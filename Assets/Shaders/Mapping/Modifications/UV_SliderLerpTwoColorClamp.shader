    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //2. SATURATE --> clamps 't' between 0 and 1. InverseLerp returns a 't' that could be outside range of 0 to 1. SATURATE applies a clamp to returned 't'
                float t = InverseLerp(_ColorStart, _ColorEnd, i.uv.x);
                t = saturate(t);

                //we can still do the frac opperator in case t is still decreasing to the left of 1, or increasing to the right of 0.
                //this will makes the _colorA apply to values above 1 instead of _colorB.
                //t = frac(t);


                //3. smoothstep --> combinatino between a inverse lerp, clamp and q-bit function.
                float4 outColor = lerp(_ColorA, _ColorB, t);
                return outColor;
