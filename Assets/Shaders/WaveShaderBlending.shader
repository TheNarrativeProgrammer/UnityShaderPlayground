Shader "Unlit/WaveShaderBlending"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        //Quene --> if it renders before or after some other shader. This is the order things are rendered in. Skybox 1st, Opaque 2nd, transpart 3rd, Overlays 4th.
        //RenderType --> tag for post process effects
        //This is paired with Blend mode. 
        ...
        Pass
        {
        //CULLING --> define what faces should be rendered.  Culling is the removal of something. If Culling Back, then the back face won't be rendered.
            //back faces (default)
            //front faces
            //both back and front (Cull Off)
        Cull Off


        //WRITE TO DEPTH BUFFER --> turn off. On by default. This controls if shader writes do Depth buffer and will cause object behind it, within depth BUFFER
        //to not render
        ZWrite Off 

        //READ FROM DEPTH BUFFER --> even if the shader doesn't write to depth buffer, it will still read from it, meaning it still test the depth buffer
        //ZTest LEqual (default) --> if depth of this object is LESS OR EQUAL to depth already written into depth buffer, then show it
        //ZTest Always --> allways draw regardless of current depth buffer value
        //ZTest GEqual --> only draw object if it's behind something, and don't draw if in front of something'
            ...
            //BLENDING MODES --> put before CGPROGRAM
            ...
            //INPUT STRUCT
            //different input Stuct - like appdate it's data 'from application to shader - contains all vertex data we're feeding into shader from CPU (short for application to vertex)'
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // //use normal of surface. If that vertor is either pointing entirely up or entirely down, then multiply by 0.
